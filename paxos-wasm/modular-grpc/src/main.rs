...
use standalone_config::NodeRole;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if config.node.role == 1 {
            let node: proposer::paxos_bindings::paxos::default::paxos_types::Node = config.node.clone().try_into()?;
            let nodes: Vec<proposer::paxos_bindings::paxos::default::paxos_types::Node> = config.remote_nodes.clone().iter()
            ...
            let paxos_wasmtime = Arc::new(ProposerPaxosWasmtime::new(node.clone(), nodes.clone(), is_leader, run_config).await?);
            ...
            let node: acceptor::paxos_bindings::paxos::default::paxos_types::Node = config.node.clone().try_into()?;
            let nodes: Vec<acceptor::paxos_bindings::paxos::default::paxos_types::Node> = config.remote_nodes.clone().iter()
                .map(|node| node.clone().try_into())
                .collect::<Result<Vec<_>, _>>()?;
            ...
            let paxos_wasmtime = Arc::new(AcceptorPaxosWasmtime::new(node.clone(), nodes.clone(), is_leader, run_config).await?);
            ...
            let node: learner::paxos_bindings::paxos::default::paxos_types::Node = config.node.clone().try_into()?;
            let nodes: Vec<learner::paxos_bindings::paxos::default::paxos_types::Node> = config.remote_nodes.clone().iter()
                .map(|node| node.clone().try_into())
                .collect::<Result<Vec<_>, _>>()?;
            ...
            let paxos_wasmtime = Arc::new(LearnerPaxosWasmtime::new(node.clone(), nodes.clone(), is_leader, run_config).await?);
