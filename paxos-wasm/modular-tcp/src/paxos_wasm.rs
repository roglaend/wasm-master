...
pub struct PaxosWasmtime {
    ...
    pub bindings: bindings::PaxosTcpWorld,
...
impl PaxosWasmtime {
    pub async fn new(
        ...
        let final_bindings =
            bindings::PaxosTcpWorld::instantiate_async(&mut store, &composed_component, &linker)
                .await?;

        let guest = final_bindings.paxos_default_tcp_server();
        let resource = guest.tcp_server_resource();
    ...
    pub fn guest<'a>(&'a self) -> &'a bindings::exports::paxos::default::tcp_server::Guest {
        self.bindings.paxos_default_tcp_server()
    }

    pub fn resource<'a>(
        &'a self,
    ) -> bindings::exports::paxos::default::tcp_server::GuestTcpServerResource<'a> {
        self.guest().tcp_server_resource()
    }
}
