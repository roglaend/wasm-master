...
cargo build --release -p client-grpc
...
for (( i=1; i<=NUM_SLOTS; i++ )); do
    ...
    RUST_LOG=$LOG_LEVEL ./target/release/client-grpc --server-addr "$LEADER_NODE_ADDR" --value "value $i"
    echo "------------------------"
    sleep 0.5
done
...
RUST_LOG=$LOG_LEVEL ./target/release/client-grpc --server-addr "$NON_LEADER_NODE_ADDR"

# Fetch log entries from the leader node starting with last_offset = 0.
echo "Fetching logs from leader node (last_offset = 0)..."
RUST_LOG=$LOG_LEVEL ./target/release/client-grpc --server-addr "$LEADER_NODE_ADDR" --fetch-logs --last-offset 0
